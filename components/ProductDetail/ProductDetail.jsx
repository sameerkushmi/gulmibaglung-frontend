...
import { FaHeart, FaRegHeart, FaShippingFast, FaShieldAlt, FaWhatsapp } from "react-icons/fa";
...
export default function ProductDetail({ slug }) {
  ...
  const { addToCart, wishlist, toggleWishlist, user, USD_RATE, EURO_RATE, AUS_RATE, currency, currencyConfig } = useAuth();
  ...
  if (!product) return <div className="h-screen flex items-center justify-center text-gold-500">Product not found</div>;
  ...
  const handleBuyNow = (product) => {
    ...
    // Direct redirect to checkout with productId and buyNow flag
          ...
          <div className="lg:col-span-5 flex flex-col justify-center">
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <span className="text-gold-500 uppercase tracking-[0.3em] text-xs font-bold">Fine Jewelry</span>
                <button
                  onClick={() => toggleWishlist(product._id)}
                  className="text-white/60 hover:text-red-500 transition-colors"
                >
                  {wishlist.includes(product._id) ? <FaHeart size={24} className="text-red-500" /> : <FaRegHeart size={24} />}
                </button>
              </div>
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex text-xs text-yellow-500 gap-1">
                  {"★★★★★".split("").map((s, i) => (
                    <span key={i} className={i >= product.averageRating ? "opacity-30" : ""}>{s}</span>
                  ))}
                  <span className="text-white/40 ml-2">({product.reviewsCount} Reviews)</span>
                </div>
              </div>

              {/* Specifications Grid */}
              <div className="grid grid-cols-2 gap-y-6 gap-x-4 border-y border-white/10 py-8 mb-8">
                <SpecItem icon={<IoDiamondOutline />} label="Material" value={product.material} />
                <SpecItem icon={<FaShieldAlt />} label="Purity" value={product.purity} />
                <SpecItem icon={<span className="text-[10px] font-bold">GR</span>} label="Weight" value={`${product.weight}g`} />
                <SpecItem
                  icon={<div className={`w-2 h-2 rounded-full ${product.stock > 0 ? "bg-green-500" : "bg-red-500"}`} />}
                  label="Availability"
              ...
              <div className="mt-8 flex items-center justify-between text-[10px] text-white/40 tracking-widest uppercase">
                <div className="flex items-center gap-2"><FaShippingFast /> Free Express Shipping</div>
                <div className="flex items-center gap-2"><FaShieldAlt /> Warranty</div>
              </div>
            </motion.div>
          </div>
        </div>
...
function SpecItem({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-[10px] uppercase text-white/40 tracking-tighter">{label}</p>
